...
def preprocess_toppings(toppings, min_relevant_stats=1):
    """
    Filter toppings based on relevance to Cooldown and ATK_SPD.
...
def find_all_valid_combos(chocolate_toppings):
    """Find valid combinations for Ferret Cookie."""
    min_cd = 38.5  # Minimum cooldown target
    min_aspd = 9.2  # Minimum attack speed
    cd_tolerance = 0.1  # Allow small deviation from target
    
    # Base stats from set bonus
    base_cd = 30.4
    base_aspd = 0
    ...
    # Try all 5 Chocolate combinations
    for combo in combinations(chocolate_toppings, 5):
    ...
    # Sort by CD first (highest), then by ATK_SPD
    valid_combos.sort(key=lambda x: (x['total_cd'], x['total_aspd']), reverse=True)
...
def print_detailed_results(valid_combos, combinations_checked, time_taken):
    ...
    for i, result in enumerate(valid_combos[:5], 1):
        ...
        print(f"CD: {result['total_cd']:.1f}, ATK_SPD: {result['total_aspd']:.1f}, ATK: {result['total_atk']:.1f}")
        print("-" * 50)
        print(f"{'Type':<12}{'ATK':<8}{'CD':<8}{'ASPD':<8}")
        print("-" * 50)
