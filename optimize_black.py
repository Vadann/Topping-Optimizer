...
def preprocess_toppings(toppings, min_relevant_stats=2):
    """
    Filter toppings based on relevance to Cooldown, Crit, and ATK.
...
def find_all_valid_combos(jelly_toppings):
    """Find valid combinations for 5 Apple Jelly strategy."""
    base_crit = 45.0
    base_atk = 30.0
    min_cd = 8.2
    ...
    print(f"- Then maximize Crit (Base: {base_crit}%)")
    print(f"- Then maximize ATK (Base: {base_atk})")
    
    # Try all possible combinations of 5 jelly toppings
    for jelly_combo in combinations(jelly_toppings, 5):
        ...
        # Skip if CD requirement not met
        if total_cd < min_cd:
            continue
    ...
    # Sort by priority: Crit > ATK (after meeting CD minimum)
    valid_combos.sort(key=lambda x: (
        round(x['total_crit'], 1),   # Highest Crit (primary)
        round(x['total_atk'], 1)     # Highest ATK (secondary)
    ), reverse=True)
