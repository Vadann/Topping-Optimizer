...
def find_all_valid_combos(chocolate_toppings):
    """Find ALL valid combinations and sort them by total Cooldown."""
    base_cd = 15 + 5 + 11.3
    base_aspd = 0
    ...
    # Try all possible combinations of 5 chocolate toppings
    for choc_combo in combinations(chocolate_toppings, 5):
        ...
        # Store all valid combinations that meet ASPD requirement
    ...
    # Sort all valid combinations by Cooldown
    valid_combos.sort(key=lambda x: x['total_cd'], reverse=True)
...
def print_detailed_results(valid_combos, combinations_checked, time_taken):
    ...
    for i, result in enumerate(valid_combos[:5], 1):
        combo = result['combo']
        print(f"\n{i}. Combination (CD: {result['total_cd']:.1f}, "
